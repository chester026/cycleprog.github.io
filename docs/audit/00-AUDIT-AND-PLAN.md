...
> | 5 — аппка: слой данных и декомпозиция | 🔶 T-5.1–T-5.4 выполнены, код на машине, ждёт ручного теста; T-5.5 частично | 18.09.2026 | **T-5.1**: TanStack Query + persist в AsyncStorage (`src/data/`: queryClient, keys, 30+ хуков и мутаций), один `HealthProvider`; `AppDataContext`, `utils/cache.ts`, `analyticsSnapshot.ts`, `goalsCache.ts` удалены; `grep AsyncStorage src` → только `queryClient` (персист), `api.ts` (миграция токена), `session.ts` (чистка legacy-ключей), `healthService.ts` (флаг подключения), `i18n`. Клиентская часть refresh-токенов: shared API-клиент делает `/api/auth/refresh` при 401 (дедуп конкурентных), пара хранится в Keychain, SSE коуча тоже обновляет токен. `jwt-decode` удалён. **T-5.2**: `RootStackParamList`, типизированные навигаторы/хуки, `linking`, `deepLinks.ts` (RN-`URL` не парсит custom scheme — свой парсер, 12 тестов), `requestId` для «Discuss with Coach», `resolvePostAuthRoute`. **T-5.3**: `components/analysis/*`, Power/Heart/Speed/Cadence/FTP 3195 → 1532 строк. **T-5.4**: `src/theme/` (токены, `useTheme`, `makeStyles`, ESLint-warn на hex вне темы); экраны: Garage 1708→228, GoalDetails 1377→286, Calendar 1318→331, Analysis 838→245, RideAnalytics 962→250, CoachChat 884→500, Onboarding 722→297; `GoalAssistantScreen` (мёртвый) удалён; ShareStudio: один `BackgroundPicker(variant)`, `TemplateFrame`, `format.ts` (3415 → 2546). Коуч переведён на контракт `{message}`. KnowledgeCenter и ряд строк — в i18n. **T-5.5 частично**: **Skia возвращена** по решению владельца 18.09 (BlobOrb без Skia выглядел плохо; A-29 закрыт как «не делать»), depcheck чист; HealthKit: `isConnected` = разрешение выдано (как раньше; вариант «по факту данных» ломал карточки готовности коуча на устройствах без данных), добавлен отдельный `hasAnyData`; iOS-only пункты скрыты на Android. Тесты аппки: 2 → 253 (jest + RNTL), `tsc` чистый. Остаток T-5.5: Sentry RN + source maps, Maestro smoke, `eslint-plugin-i18next`, `BikeGarageScreen` (740 строк), Sign in with Apple — отложен владельцем. Ручные шаги — §5.1. |
> | 6 — веб | 🔶 T-6.1–T-6.3, T-6.5 (кроме единого языка UI и self-host шрифтов) выполнены, код на машине, ждёт ручного теста; T-6.4 — решения зафиксированы | 18.09.2026 | **Решения владельца 18.09 (T-6.4):** ручные Rides, Events, WeatherBlock — **оставить** и модернизировать; **NutritionPage удалена** (калькуляторы в Garage/GoalAssistant остаются); **Checklist возрождён**: блок `ChecklistPreview` в Garage под My Rides (карточки-секции с горизонтальным скроллом → `/checklist`), пункт Checklist в сайдбаре; админка — lazy-чанк в SPA за `is_admin`; токен — access в памяти + refresh в localStorage; фичи между вебом и аппкой не переносить. **T-6.1**: `src/auth/AuthProvider` — единственное место с токеном, миграция старого `token` один раз, `createBrowserRouter` + `errorElement` + 404, ProtectedRoute-layout, `jwt-decode` удалён; страницы forgot/reset password. **T-6.2**: `src/data/` (TanStack Query + persist, ~40 хуков), удалены `cache.js`, `cacheCheckup`, `cacheConstants`, `CacheStatus`, `imageCache`; `MetaGoalRow` без N+1; HR-зоны и прогресс целей — только с сервера. **T-6.3**: `src/ui` (Modal/ConfirmDialog/useConfirm/Toast/ErrorMessage/Loader/Button, a11y), 7 графиков → `charts/TrendChart` (987 → 319), страницы: WeeklyTrainingCalendar 1156→167, AdminPage 852→57 (+ вкладка AI Usage), TrainingsPage 785→371, GoalAssistant 689→425, GoalDetail 683→260, OnboardingModal 680→285, ProfilePage 544→238 (+ Danger Zone: удаление аккаунта), AnalysisPage 566→99 с ленивым монтажом графиков, LandingPage 601→93; все `alert/confirm` → диалоги; ни одного файла > 600 строк. **T-6.5**: ESLint 84 → 0 и обязателен в CI; vitest 0 → 200 тестов; Playwright smoke (login → garage → analysis → 404 → logout) + job `web-e2e` с реальным сервером и Postgres; SEO-мета/OG/manifest, семантические landmarks на лендинге. Не сделано: единый язык UI (ru/en микс), self-host шрифтов, `eslint-plugin-jsx-a11y` (нужна установка). Ручные шаги — §6.1. |
> | 7 — контракт и защита от регресса | ✅ T-7.1 и T-7.2 выполнены (18.09), код на машине; хвосты T-5.5/T-6.5 закрыты; ждёт `npm install` + ручного теста | 18.09.2026 | **T-7.1**: `@bikelab/shared/api` — `defineEndpoint`/`callEndpoint`/`buildPath`, реестр `contract/<domain>.ts` (20 доменов, 109 маршрутов; вне контракта только SSE-чат коуча и бинарный прокси Strava); сервер — `contract(def)` на каждом `/api`-маршруте вместо `validateBody`, `uncontracted(reason)` для исключений, `CONTRACT_VALIDATE_RESPONSES=1` проверяет каждый 2xx-ответ (в CI включён — поймал nullable-поля профиля, `Date` из pg, NULL `goal_type`/`source` у AI-подцелей); `test/contract.inventory.test.js` ходит по живому Express-роутеру (маршрут без контракта или контракт без маршрута = красный CI), `npm run routes` печатает список из кода. Клиенты: аппка — `src/data/api.ts`, `apiFetch` удалён, 0 строковых путей; веб — `src/data/api.js` + `call()`, `apiFetch` остался только для мёртвых admin-ручек (`/api/strava/tokens`, `/api/database/*` — маршрутов на сервере нет). **Решение**: пути/поля НЕ переименовывались до снятия `LEGACY_MOBILE_COMPAT`. **T-7.2**: `shared/calc` 100 % по всем метрикам (порог в vitest-конфиге + CI); jscpd — 2.29 % при пороге 6 (обязателен); knip — report-only, находки в `docs/audit/quality-gates.md`; eslint: веб `--max-warnings 0`, аппка 0 ошибок обязательно (≈930 warn — hex-цвета вне темы, лимит 1000), сервер 39 warn report-only; `tsc` в shared и аппке; `npm audit` report-only (12 advisories, все из `@getbrevo/brevo`→`request` и `imagekit`→`uuid`); Node 22 (`engines`, `.nvmrc`, `.node-version`, CI). **Хвосты T-5.5**: Sentry RN (`src/monitoring/sentry.ts`, no-op без `SENTRY_DSN`, `docs/sentry.md`), Maestro flows (`.maestro/*.yaml`, `docs/maestro.md`, testID на экранах), `eslint-plugin-i18next` как error (138 литералов переведены, 26 новых ключей; в WeatherBlock был захардкоженный русский), `BikeGarageScreen` 740→274 (`screens/BikeGarage/*`). **Хвосты T-6.5**: веб только на английском (23 файла, `en-GB` даты); шрифты self-host (`@fontsource-variable/inter`, `material-symbols`); `jsx-a11y` recommended как error, 17+~60 правок. Ручные шаги — §7.1. |
>
...
>
> **§6.1 Ручные шаги после фазы 6 (владелец):** в корне `npm install` (веб: `@tanstack/*`, RTL/jsdom, `@playwright/test`; удалён `jwt-decode`). Локально: `cd react-spa && npm run lint && npm test && npm run build`; e2e локально требует Postgres (`npm run test:e2e`), в CI гоняется само. Сессия: старый `token` из localStorage мигрируется автоматически, перелогин не нужен. Проверить: логин email/Strava, F5 — сессия жива; Garage; Analysis (графики появляются при скролле, Power 50/50); Trainings (фильтры, «Show more», модалка анализа); Goal Assistant (+ AI generate, 429 → тост); детали цели (табы, календарь тренировок, новый блок рекомендаций); Profile (смена email → 409 тостом, unlink Strava, Danger Zone); My Rides (теперь есть редактирование), Events, Checklist, Maintenance; Admin (5 вкладок, AI Usage); лендинг; `/forgot-password`. Изменения поведения: убран «Remember me»; загруженный GPX не переживает F5; в модалках общий тёмный оверлей `src/ui/Modal`.
>
> **§5.1 Ручные шаги после фазы 5 (владелец):** `cd BikeLabApp && npm install && (cd ios && pod install)` — добавлены `@tanstack/react-query`, `@tanstack/query-async-storage-persister`, `@tanstack/react-query-persist-client`, dev `@testing-library/react-native`; удалён `jwt-decode` (Skia остаётся). Затем `npm run typecheck && npm test`, Metro с `--reset-cache`. Один раз потребуется перелогиниться (старый токен без `refreshToken`). Проверить в симуляторе: логин по deep link → Garage; выход/вход; Garage (pull-to-refresh, hero, гараж-фото, погода, планы, ачивки, нутрициология); Analysis (все секции, оверлей); детали заезда (потоки, качество, цели, «Discuss with Coach» дважды с разных заездов); Goals → GoalDetails (3 таба, complete/delete, библиотека тренировок); Calendar (месяц/неделя/день, редактирование/удаление, Apple-sync на iOS); Coach (стрим ответа, история с сервера, список диалогов); Profile → интеграции (Strava/Oura/Apple Health, на Android нет Apple Health); Share Studio (шаблоны A–F, фоны, экспорт 1080×1920).
>
...
### 7.1 Ручные шаги владельца после фазы 7

1. Корень: `npm install` (jscpd, knip, `@vitest/coverage-v8`; веб: `@fontsource-variable/inter`, `material-symbols`, `eslint-plugin-jsx-a11y`). `BikeLabApp`: `npm install` (`@sentry/react-native`, `eslint-plugin-i18next`) + `pod install`.
2. Node 22 локально (`.nvmrc`); Render подхватит `.node-version`.
3. Sentry в аппке включается переменной `SENTRY_DSN` (см. `BikeLabApp/docs/sentry.md`); без неё — no-op.
4. Maestro: поставить CLI, вписать `appId` в `.maestro/*.yaml`, прогнать `login → garage/analysis/goals/coach` на симуляторе (`BikeLabApp/docs/maestro.md`).
5. Проверить в браузере: шрифты (Inter, иконки Material Symbols) без Google Fonts; клавиатурная навигация по кликабельным карточкам; даты `dd/mm`.
6. Проверить в симуляторе: логин/онбординг, гараж + экран велосипеда (декомпозирован), цели, календарь, чеклист, коуч, Oura — все запросы идут через контракт.
7. После выхода новой сборки в App Store: убрать `LEGACY_MOBILE_COMPAT` на Render и ветки `legacyMobile*`; затем `ACCESS_TOKEN_TTL=1h`.
8. Отложенные находки: `calculateConsistency` (бонус текущей недели никогда не начисляется — см. комментарий в `skills.ts`), мёртвые admin-вкладки Strava tokens / Database, `manifest.webmanifest` без иконок, advisories `@getbrevo/brevo`/`imagekit` (мажорные бампы), knip-находки.
