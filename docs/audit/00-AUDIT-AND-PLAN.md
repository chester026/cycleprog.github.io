...
> | 1–7 | ⏳ | | |
>
> **Стратегия тестов (решение 16.09):** тесты пишутся по ходу, а не в конце. Правило для каждого задания: новая middleware / чистая функция / helper — обязателен unit-тест в том же PR (сервер: `vitest`, `server/test/`; каркас поднят в фазе 0 — 26 тестов). Интеграционные тесты маршрутов (supertest + testcontainers) и клиентские тесты (Jest/Vitest/Maestro) — в T-1.7, T-5.5, T-6.5; покрытие `shared/calc` 100 % — в T-7.2.
>
