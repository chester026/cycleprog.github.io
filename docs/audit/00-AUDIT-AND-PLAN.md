...
> | 4 — декомпозиция сервера | 🔶 T-4.1 выполнен, код на машине; T-4.2–T-4.5 ⏳ | 17.09.2026 | `server.js` 5 403 → 238 строк (только bootstrap: middleware, статика, монтирование роутеров, SPA-fallback, lifecycle). 24 роутера в `routes/`, логика в `services/`, SQL в `repositories/` (12 файлов). Извлечение делали 15 параллельных агентов в git-worktree по доменам; каждый домен получил smoke-тесты на реальном Postgres: интеграционных тестов 60 → 224, unit 138 → 144. Попутно найдены и исправлены: регрессия T-1.1 в `getImageKitConfig` (локальная `const config` затеняла модуль → загрузка картинок падала 500); онбординг молча не сохранялся, если строки `user_profiles` ещё не было; тест-инфра: `DATABASE_URL` теперь переписывается на scratch-БД (раньше тесты втихую ходили в БД из URL). Остаток DoD «ни одного `pool.query` в `routes/`»: ~30 вызовов в admin/analyticsSnapshot/media/metaGoals/oura/skills/skillsHistory — переносятся в T-4.2 вместе с транзакциями. Ручные шаги — §4.1. Sign in with Apple (T-4.5) — по решению владельца 17.09 откладывается на потом. |
> | 5–7 | ⏳ | | |
>
