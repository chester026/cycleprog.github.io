...
export async function startStravaLogin() {
  const { url } = await call(auth.stravaStart, { query: { client: 'web' } });
...
// call()'s Authorization header is what ties the mint to `req.userId` on
...
export async function startStravaLink({ popup = false } = {}) {
  const { url } = await call(auth.stravaLinkStart, { query: { client: 'web' } });
  if (popup) {
