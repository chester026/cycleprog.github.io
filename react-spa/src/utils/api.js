...
// §5.7, §5.8, §6.1 row `api/client.ts`). Public surface (`isApiError`) is
// unchanged so every existing call site keeps working untouched — only the
// token source and the 401/refresh handling changed (T-6.1, W-03/W-24/W-25):
// this file no longer knows where tokens live at all, `AuthProvider.jsx`
// does, and wires itself in via `setAuthHandlers`.
...
// as a plain module-level object (not a React context) because `call`
// itself is invoked from plain functions outside any component (utils/*.js).
...
// per-domain endpoint maps + `callEndpoint`). Hooks/pages call
// `call(goals.list, {...})` — the path string lives once, in the contract
// def, and the input/response are validated against its zod schemas.
// Re-exported (with the domain maps) from `src/data/api.js` so call sites
