...
import { useActivities, useProfile } from '../data/hooks';
...
import GoalsHero from './goals/GoalsHero';
import MetaGoalList from './goals/MetaGoalList';
...
// T-6.3 part 2: GoalAssistantPage split into GoalsHero (blob/heading/partner
// logos) -> AiGoalGenerator (input + useGenerateAiGoals) and MetaGoalList
// (tabs + the meta-goal cards, self-fetching via useMetaGoals). This file
// now only owns the VO2max/Nutrition calculators, which weren't part of
// that split (no server data involved — they're pure client-side unit
// calculators, same as before). The unused `calculateMetaProgress` dead
// function (no call site) and the `isRelevantToCycling` helper (moved to
// `./goals/lib.js`, used by AiGoalGenerator) are gone from here.
export default function GoalAssistantPage() {
  const { data: activitiesData } = useActivities();
  const activities = activitiesData || [];
  const { data: userProfile } = useProfile();

  // Display name for the hero greeting — comes from the auth context now
  // instead of a separate jwtDecode(token) read.
  const { user } = useAuth();
  ...
  // Автозаполнение полей калькулятора из профиля (once it loads)
  useEffect(() => {
    if (userProfile?.weight || userProfile?.age || userProfile?.gender) {
      setVo2maxData((prev) => ({
        ...prev,
        weight: prev.weight || userProfile.weight || '',
  ...
  return (
    <div className="goal-assistant-page">
      <GoalsHero userName={userName} activities={activities} />
