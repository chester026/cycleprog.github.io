// Extracted from BikeGarageScreen.tsx (screen decomposition, T-5.5 /
// GUIDE-5b). Pixel-identical to the original inline header.
import React from 'react';
import {Text, TouchableOpacity, View} from 'react-native';
import {useTranslation} from 'react-i18next';
import {makeStyles} from '../../theme';

interface HeaderProps {
  onBack: () => void;
}

export const BikeGarageHeader: React.FC<HeaderProps> = ({onBack}) => {
  const {t} = useTranslation();
  return (
    <View style={styles.header}>
      <TouchableOpacity onPress={onBack} hitSlop={{top: 12, bottom: 12, left: 12, right: 12}}>
        <Text style={styles.backArrow}>{'‹'}</Text>
      </TouchableOpacity>
      <Text style={styles.headerTitle}>{t('bikeGarage.title')}</Text>
      <View style={styles.spacer} />
    </View>
  );
};

const styles = makeStyles(theme => ({
  header: {
    flexDirection: 'row',
    alignItems: 'center',
    justifyContent: 'space-between',
    paddingTop: 60,
    paddingHorizontal: theme.spacing[16],
    paddingBottom: theme.spacing[8],
  },
  backArrow: {fontSize: 32, color: theme.colors.text.primary, lineHeight: 34, fontWeight: '300'},
  headerTitle: {fontSize: 17, fontWeight: '600', color: theme.colors.text.primary, letterSpacing: -0.3},
  spacer: {width: 28},
}));
