...
export const ProfileScreen: React.FC<{navigation: AppNavigationProp}> = ({navigation}) => {
  const {t, i18n} = useTranslation();
  const theme = useTheme();
  // T-5.1/A-17/A-34 (docs/audit/layers/02-bikelabapp.md): this screen used to
  // own its own loading/useState around a manual
  // apiFetch('/api/user-profile') — now it shares the useProfile()
  // query/cache entry like every other profile consumer.
  ...
  return (
    <ScrollView style={styles.container} contentContainerStyle={styles.scrollContent}>
      ...
      <View style={styles.section}>
        ...
        />
        {/* A-40: Apple Health is iOS-only (HealthKit has no Android
            equivalent) — hide the row entirely rather than showing an
            integration that can never connect. */}
        {Platform.OS === 'ios' && (
          <SettingsItem
            icon=""
        ...
        <SettingsItem
          ...
          onPress={() => navigation.navigate('OuraIntegration')}
          hideDivider={true}
        />
      </View>
